...
module JSON
	...
	#
	# From forked repo https://github.com/maweeks/NeatJSON
	#
	...
	# @option opts [Boolean] :flatten_simple_arrays  (false) Keep simple arrays on one line..
	...
	def self.neat_generate(object,opts={})
		...
		opts[:flatten_simple_arrays] ||= false
			...
			memoizer[[o,indent]] ||= case o
				...
				when Array
					if o.empty?
						...
						simple_array = true
						for i in 0 ... o.length
							if (o[i].instance_of? Array) || (o[i].instance_of? Hash)
								simple_array = false
							end
						end
						pieces = o.map{ |v| build[v,''] }
						one_line = "#{indent}[#{apad}#{pieces.join comma}#{apad}]"
						if !opts[:wrap] || (one_line.length <= opts[:wrap]) || (simple_array && opts[:flatten_simple_arrays])
							one_line
						elsif opts[:short]
							indent2 = "#{indent} #{apad}"
